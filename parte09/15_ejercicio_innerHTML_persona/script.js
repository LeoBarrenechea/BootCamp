let personas = {
    ...
    person = prompt("nombre de la persona que quieres ver los datos:");

    let body = document.querySelector("body");
    let all = document.querySelector("*");

    all.style.backgroundColor = personas[person]["eyeColor"];
    body.style.backgroundColor = personas[person]["eyeColor"];
    body.innerHTML = '<div id="main"></div>';
    div = document.querySelector("#main");
    
    if (personas[person] == undefined)
        div.innerHTML =
        '<h1>no existe ese nombre en la base de datos</h1>';
    else
    {
        div.innerHTML +=`
        <img src="${personas[person]["picture"]}"></img>
        <h1> ${person}</h1>
        <p> ${personas[person]["age"]}</p>
        <p> ${personas[person]["email"]}</p>
        <p> ${personas[person]["address"]}</p>
        `;
    }
